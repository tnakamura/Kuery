...
{
    public class ContainsTest : IClassFixture<SqlClientFixture>
    {
        readonly SqlClientFixture fixture;

        public ContainsTest(SqlClientFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table {nameof(ContainsTestObj)} (
                        {nameof(ContainsTestObj.Id)} integer primary key identity,
