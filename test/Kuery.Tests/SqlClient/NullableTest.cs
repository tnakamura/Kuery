...
{
    public class NullableTest : IClassFixture<SqlClientFixture>
    {
        readonly SqlClientFixture fixture;

        public NullableTest(SqlClientFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table {nameof(NullableIntClass)} (
                        {nameof(NullableIntClass.ID)} integer primary key identity,
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table {nameof(NullableFloatClass)} (
                        {nameof(NullableFloatClass.ID)} integer primary key identity,
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table {nameof(StringClass)} (
                        {nameof(StringClass.ID)} integer primary key identity,
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table {nameof(NullableEnumClass)} (
                        {nameof(NullableEnumClass.ID)} integer primary key identity,
