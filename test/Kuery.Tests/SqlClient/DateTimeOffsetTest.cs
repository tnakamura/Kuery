...
{
    public class DateTimeOffsetTest : IClassFixture<SqlClientFixture>
    {
        readonly SqlClientFixture fixture;

        public DateTimeOffsetTest(SqlClientFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table {nameof(DtoTestObj)} (
                        {nameof(DtoTestObj.Id)} integer primary key identity,
