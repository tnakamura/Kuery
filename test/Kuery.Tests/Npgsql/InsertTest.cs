...
{
    public class InsertTest : IClassFixture<NpgsqlFixture>, IDisposable
    {
        readonly NpgsqlFixture fixture;

        readonly global::Npgsql.NpgsqlConnection connection;

        public InsertTest(NpgsqlFixture fixture)
        {
        ...
        private void DropTestTables(global::Npgsql.NpgsqlConnection connection)
        {
        ...
        private void CreateTestTable(global::Npgsql.NpgsqlConnection connection)
        {
            DropTestTables(connection);
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table if not exists ""{nameof(InsertTestObj)}"" (
                        ""{nameof(InsertTestObj.Id)}"" integer primary key generated by default as identity,
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table if not exists ""{nameof(OneColumnObj)}"" (
                        ""{nameof(OneColumnObj.Id)}"" integer primary key generated by default as identity
                    );";
                cmd.ExecuteNonQuery();
            }
            using (var cmd = connection.CreateCommand())
        ...
        public void InsertAllFailureOutsideTransaction()
        {
            ...
            Assert.Throws<global::Npgsql.PostgresException>(() =>
            {
                connection.InsertAll(testObjects);
            });
