...
{
    public class ByteArrayTest : IClassFixture<NpgsqlFixture>
    {
        readonly NpgsqlFixture fixture;

        public ByteArrayTest(NpgsqlFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(ByteArrayClass)}"" (
                        ""{nameof(ByteArrayClass.ID)}"" integer primary key generated by default as identity,
                        ""{nameof(ByteArrayClass.bytes)}"" bytea null
                    );";
                cmd.ExecuteNonQuery();
            }
        }
