...
{
    public class GuidTest : IClassFixture<NpgsqlFixture>, IDisposable
    {
        readonly NpgsqlFixture fixture;

        public GuidTest(NpgsqlFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(GuidTestObj)}"" (
                        ""{nameof(GuidTestObj.Id)}"" uuid primary key,
                        ""{nameof(GuidTestObj.Text)}"" varchar(64) null
                    );";
                cmd.ExecuteNonQuery();
            }
        }
