...
{
    public class AsyncTest : IClassFixture<NpgsqlFixture>
    {
        readonly NpgsqlFixture fixture;

        public AsyncTest(NpgsqlFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(AsyncCustomer)}"" (
                        ""{nameof(AsyncCustomer.Id)}"" integer primary key generated by default as identity,
                        ""{nameof(AsyncCustomer.FirstName)}"" varchar(64) not null,
                        ""{nameof(AsyncCustomer.LastName)}"" varchar(64) null,
                        ""{nameof(AsyncCustomer.Email)}"" varchar(64) null
                    );";
                cmd.ExecuteNonQuery();
            }
        }
