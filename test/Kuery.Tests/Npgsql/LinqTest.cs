...
{
    public class LinqTest : IClassFixture<NpgsqlFixture>
    {
        readonly NpgsqlFixture fixture;

        public LinqTest(NpgsqlFixture fixture)
        {
            this.fixture = fixture;
        }

        void CreateTables(global::Npgsql.NpgsqlConnection connection)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(Product)}"" (
                        ""{nameof(Product.Id)}"" integer primary key generated by default as identity,
                        ""{nameof(Product.Name)}"" varchar(50) not null,
                        ""{nameof(Product.Price)}"" decimal not null,
                        ""{nameof(Product.TotalSales)}"" integer not null
                    );";
                cmd.ExecuteNonQuery();
            }
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(Order)}"" (
                        ""{nameof(Order.Id)}"" integer primary key generated by default as identity,
                        ""{nameof(Order.PlacedTime)}"" timestamp not null
                    );";
                cmd.ExecuteNonQuery();
            }
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(OrderHistory)}"" (
                        ""{nameof(OrderHistory.Id)}"" integer primary key generated by default as identity,
                        ""{nameof(OrderHistory.OrderId)}"" integer not null,
                        ""{nameof(OrderHistory.Time)}"" timestamp not null,
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(OrderLine)}"" (
                        ""{nameof(OrderLine.Id)}"" integer primary key generated by default as identity,
