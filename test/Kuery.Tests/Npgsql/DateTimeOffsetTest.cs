...
{
    public class DateTimeOffsetTest : IClassFixture<NpgsqlFixture>
    {
        readonly NpgsqlFixture fixture;

        public DateTimeOffsetTest(NpgsqlFixture fixture)
        {
            ...
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = $@"
                    create table ""{nameof(DtoTestObj)}"" (
                        ""{nameof(DtoTestObj.Id)}"" integer primary key generated by default as identity,
                        ""{nameof(DtoTestObj.Name)}"" varchar(50) null,
                        ""{nameof(DtoTestObj.ModifiedTime)}"" timestamp with time zone null
                    );";
                cmd.ExecuteNonQuery();
            }
        }
